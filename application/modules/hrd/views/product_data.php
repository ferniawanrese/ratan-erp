...
<table class="responsive table table-striped table-bordered table-hover" style = "padding-top:20px;">
	...
	<tbody>
			...
			<?php if($product_data):?>		
			<?php foreach($product_data as $keys):?>
				<tr>				
					<td>
					<?php  echo $keys['product_name']; ?>	
					</td> 
					<td>					
					 <?php  echo $keys['product_code']; ?>
					</td> 
					<td>					
					 <?php  echo $keys['product_desc']; ?>
					</td> 
					<td>					
					...
					<td class="center">
							<div class="btn-toolbar row-action">
									...
									<button class="btn btn-info" title="Edit" onclick=update_product("<?php echo $keys['product_ID'];?>")><i class="icon-edit"></i></button>
									<button class="delete btn btn-danger" title="Delete" onclick=delete_post("<?php echo $keys['product_ID'];?>")><i class="icon-trash "></i></button>
...
<script type='text/javascript'>
			var options = {
				...
				onPageClicked: function changepage(e,originalEvent,type,page){
					...
					$.ajax({
					type: "POST",
					url: "<?php echo base_url('hrd/product_data/');?>" + "/" + page,

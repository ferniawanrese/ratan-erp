<ul class="breadcrumb">
	...
	<li class="active">Asset State</li>	
</ul>
		...
		<div class="row-fluid">
					<div class="content-widgets gray">
						<div class="widget-head blue clearfix">
						  <h3 class="pull-left" onclick="display_data()" style="cursor:pointer;">Asset State Data</h3>							
						</div>
															
						<div class="well col-sm-12 col-md-12">
						<form id = "form_filter"  method="post">
							<div class = "col-md-3 btn-create">
								<input type ="text" name ="search" id="search" class="form-control" placeholder="State Name">
							</div> 
							...
							<div class = "col-md-3 btn-create form-group "  >
							 <button class="btn btn-inverse btn-large icon-plus" type="button" onclick = "assetgroup_add()"  > Create</button>
							</div>
...
function display_data(){ 
	...
	$.ajax({
				type: "POST",
				url: "<?php echo base_url('asset/asset_state_data/');?>",
...
function assetgroup_add(a){
	NProgress.inc();	
	$('#btn-list').show();
	$('.btn-create').hide();
	$.ajax({
				
				url: "<?php echo base_url('asset/asset_state_add/');?>" +"/"+ a,
				success: function(data){     
...
function delete_post(a){
	...
	bootbox.confirm("Are you sure delete this item?", function (result) {
					...
					if(result == true){						
						$.ajax({
									url: "<?php echo base_url('asset/asset_state_delete/')?>/" + a,									
									success: function(data)
									{											
											display_data();
									}

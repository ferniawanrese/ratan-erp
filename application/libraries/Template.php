...
class Template {
	...
	function generate_template($pagedata, $file = false)
	{
		...
		// find out if merchant
		//$this->template['merchant'] = ($this->CI->session->userdata('groupID') == 3) ? TRUE : FALSE;
	
		$this->template['merchant:group'] = $this->CI->session->userdata('groupID') ? $this->CI->session->userdata('groupID') : FALSE;

		@$this->template['user-loggedin'] = ($this->CI->session->userdata['displayName'] != '') ? $this->CI->session->userdata['displayName'] : $this->CI->session->userdata['firstName'] . ' ' . $this->CI->session->userdata['lastName'];
		
		$this->template['is_vip'] = ($this->CI->session->userdata('memberType') == 'VIP') ? 'VIP' : '';
		$this->template['is_basic'] = ($this->CI->session->userdata('memberType') == 'BASIC') ? 'BASIC' : '';
		
		/* newsletter */
		$this->template['userID'] = $this->CI->session->userdata('userID');
		$this->template['email'] = $this->CI->session->userdata('email');
		$this->template['subscribe_email'] = $this->CI->session->userdata('subscribe_email');
		$this->template['newsletter_url'] = $this->CI->core->encode('users/newsletter');

		/* testimonial */
		$this->template['testimonial'] = $this->CI->core->get_testimonial();

		$due_date = strtotime($this->CI->session->userdata('vip_expired_date'));
		$reminder = strtotime("-1 month",$due_date);		
		$datenow = time();
		if($this->CI->session->userdata('session_user') == TRUE && $this->CI->session->userdata('memberType') == 'VIP'){
			if($reminder < $datenow){
				$this->template['is_vip_expired'] = 'Please <a href="'.base_url('users/upgrade_vip_member').'"><strong>renew</strong></a> your membership!';
			}
		}

		$this->template['uri-encoded'] = $this->CI->core->encode($this->CI->uri->uri_string());
		
		$this->template['page:baseurl'] = rtrim(base_url(), '/');
		
		//ihdi currency
		$this->template['currency:separator'] = $this->CI->site->config['currency_format_separator'];
		$this->template['currency:decimal'] = $this->CI->site->config['currency_format_decimal'];
		$this->template['currency:symbol'] = $this->CI->site->config['currency_symbol'];
		$this->template['currency'] = '';
		if($this->CI->site->config['enableCurrency'] == 1) {
			$curr = $this->CI->core->get_currency();
			$ar_currency = array();
			foreach($curr as $key=>$val) {
				$ar_currency[$val['currency_code']] = $val['currency_code'];
			}
			$currency = $this->CI->session->userdata('currency_attributes');
			$this->template['currency'] = '<form method="post" id="currency">'.form_dropdown('currency',$ar_currency,$currency['name'],'onchange="document.getElementById(\'currency\').submit()"').'</form>';
		}
		
		// Add by Ryan
		if($this->CI->session->userdata('groupID') == 3)
		{
			$this->CI->load->model('merchants/merchants_model', 'mrc_model');	
			$mrcID = $this->CI->mrc_model->get_mrc_id();	
			$this->template['merchant'] = $this->CI->db->get_where('merchants',array('mrcID'=>$mrcID))->result_array();
			//print_rr($this->template['merchant']);
		}
		if($this->CI->session->userdata('groupID') == 2)
		{
			//$start_time = microtime(true);
			$this->CI->load->model('payment/payment_service_model');
			$resultFromServer = $this->CI->payment_service_model->find_last_balances($this->CI->session->userdata('userID'));
			$ewallet = 0;
			$crp = 0;
			try {
				if ($resultFromServer) {
					$ewallet = isset($resultFromServer['ewallet']) ? (is_numeric($resultFromServer['ewallet'])?$resultFromServer['ewallet']:0):0;
					$crp = isset($resultFromServer['crp']) ? (is_numeric($resultFromServer['crp'])?$resultFromServer['crp']:0):0;
				}
			} catch (Exception $ex) {
				// NO OP
			} 
			$this->template['saldo'] = number_format($ewallet);
			$this->template['crp'] = number_format($crp);
			//$end_time = microtime(true);
			//ob_end_clean();
			//echo 'Curl without handle reuse: ' . ($end_time - $start_time) . '<br>';
				
			//$this->CI->load->model('community/transactions_model','transactions');
			//$this->template['saldo'] = number_format($this->CI->transactions->get_last_user_balance($this->CI->session->userdata('userID')));	
		}
		
		$this->template['cat:deal'] = $this->deal_cat_nav();
		$this->template['cat:shop'] = $this->shop_cat_nav();
		$this->template['serverTime'] = strftime('%c');
		$this->template['user-menu'] = $this->CI->db->get_where('users_menu',array('groupID' => $this->CI->session->userdata('groupID')))->result_array(); 
		
		/*
		// load cart
		$this->CI->load->model('shop/shop_model');
		
		$data = $this->CI->shop_model->load_cart();
		...
		$this->template['cart-item'] = $total . ' item(s) - ' . currency_symbol() . ' ' . number_format((@$data['subtotal'] + @$data['postage'] + @$data['tax']), 2);
		*/
	...
	function parse_includes($body)
	{
		...
		*/
		// hajaaaar pake real file <tatangajah@gmail.com>
		foreach ($includes as $include) {
			if ($file = @file_get_contents(FCPATH.APPPATH.'views/includes/common/templates/'.$include[0].'.php')) {
				$body = str_replace('{include:'.$include[0].'}', $file, $body);
			}
		}
	...
	{		
		...
		$this->CI->load->model('deals/deals_model','deals');
		...
		$body = str_replace('{site:vip-member-fee}', number_format($this->CI->site->config['vipMemberFee'], 2), $body);
		$body = str_replace('{site:membership-agreement}', $this->CI->site->config['membershipAgreement'], $body);
		...
		// uri string encoded
		$body = str_replace('{uri:uri-encoded}', $this->CI->core->encode($this->CI->uri->uri_string()), $body);
				...
				if ($categories = $this->CI->shop->get_category_parents())
				{
					...
					foreach($categories as $nav)
					{
						...
						if ($children = $this->CI->shop->get_category_children($nav['catID']))
						{
							...
							$template['shop:categories'] .= '"><a href="'.base_url('shop/'.$nav['catSafe']).'">'.htmlentities($nav['catName'], NULL, 'UTF-8').'</a><ul class="subnav">';
							...
							foreach($children as $child)
							{
								...
								{
									$template['shop:categories'] .= 'active';
								}
								$template['shop:categories'] .= '"><a href="'.base_url('shop/'.$nav['catSafe'].'/'.$child['catSafe']).'">'.htmlentities($child['catName'], NULL, 'UTF-8').'</a></li>';
							}
						...
						else
						{
							...
							$template['shop:categories'] .= '"><a href="'.base_url('shop/'.$nav['catSafe']).'">'.htmlentities($nav['catName'], NULL, 'UTF-8').'</a>';
						}
	...
	function shop_cat_nav()
	{
		$this->CI->db->where('deleted !=', 1);
		
		return $this->CI->db->get('shop_cats')->result_array();
	}
	function deal_cat_nav()
	{
		$this->CI->db->where('deleted !=', 1);
		
		return $this->CI->db->get('deals_cats')->result_array();
	}
}
    ...
    $('.accordion-nav li').each(function () {
        if ($(this).children('ul').length > 0) {
            if ($(this).children('ul').is(":visible")) {
                $(this).prepend('<img src="' + baseurl + 'images/imgOnOpen.png" />');
            }
            else {
                $(this).prepend('<img src="' + baseurl + 'images/imgOffClosed.png" />');
            }
        }
    });
    ...
    $('.accordion-nav img').click(function() {
        if ($(this).parent('li').children('ul').html() != null) {
            ...
            if ($(this).parent('li').children('ul').css('display') == "block") {
                ...
                $(this).attr('src', baseurl + 'images/imgOffClosed.png');
            } else {
                $(this).parent('li').children('ul').show(slideSpeed);
                $(this).attr('src', baseurl + 'images/imgOnOpen.png');
            }
            return false;
        }
